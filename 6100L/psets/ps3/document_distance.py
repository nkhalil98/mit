...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    for word in all_words:
        freq1 = freq_dict1.get(word, 0)
        freq2 = freq_dict2.get(word, 0)
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    word_freqs = get_frequencies(words)
    tf = {word: freq / total_words for word, freq in word_freqs.items()}
    return tf
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for word, count in freqs.items():
        idf[word] = math.log10(n_files / count)
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    * TF-IDF(i) = TF(i) * IDF(i)
    ...
    sorted_tf_idf = sorted(tf_idf.items(), key=lambda x: (x[1], x[0]))
