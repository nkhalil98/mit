...
class TestPS5(unittest.TestCase):
    ...
    def bit_tester(self, f, test_cases):
        for inputs, expected in test_cases:
            ...
            err_msg = "Method {methodname} produced an incorrect result with inputs {inputs}. Got {actual} but expected {expected}.".format(
                methodname=f.__name__, inputs=inputs, actual=actual, expected=expected
            )
    ...
    def test_reveal_img_bw(self):
        self.assertTrue(
            self.help_check_hidden_img(
                "tester_bw_img.obj", ps5.reveal_image("hidden1.bmp")
            ),
    ...
    def test_reveal_img_rgb(self):
        self.assertTrue(
            self.help_check_hidden_img(
                "tester_rgb_img.obj", ps5.reveal_image("hidden2.bmp")
            ),
...
if __name__ == "__main__":
    ...
    suite.addTest(loader.loadTestsFromTestCase(TestPS5))
    ...
    print(output)
    print(
        "Points: {points}/{total_points}".format(
