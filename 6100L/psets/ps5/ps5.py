...
def filter(pixels_list, color):
    ...
    """
    tmatrix = make_matrix(color)
    new_pixels = []
    for pixel in pixels_list:
        new_pixel = matrix_multiply(tmatrix, pixel)
        new_pixels.append(tuple(map(int, new_pixel)))
    return new_pixels
...
def reveal_bw_image(filename):
    ...
    new_pixels = []
    for pixel in pixels:
        new_pixel = extract_end_bits(1, pixel)
        new_pixel = new_pixel * 255  # scale to 0-255
        new_pixels.append(new_pixel)
    new_im = pix_to_img(new_pixels, (width, height), "L")
    return new_im
...
def reveal_color_image(filename):
    ...
    for pixel in pixels:
        ...
        new_pixel = tuple([x * 255 / 7 for x in new_pixel])  # scale to 0-255
        new_pixel = tuple(map(int, new_pixel))
        new_pixels.append(new_pixel)
    new_im = pix_to_img(new_pixels, (width, height), "RGB")
    return new_im
